...
### Simple summary use
Based on `summarise()` function or inside a `mutate()` call.
...
## there are as many columns as things I have asked for but only one column.
```
A |> f(B) |> g(C)
is same as
g(f(A,B), c)
...
white_wine |>
  select(alcohol) |>
  mutate(n_a = median(alcohol))
```

```{r}
white_wine |>
  select(alcohol) |>
  mutate(alc_m_median = alcohol - median(alcohol),
         .keep = "none")
```

A robust statistic is difficult to manipulate. Fx you cannot change median salary of the class by simply changing one value (median is robust). You CAN change the mean salary of the class if you add an Mbappé (mean is not robust bc easy to manipulate). 

```{r}
white_wine |>
  select(alcohol) |>
  mutate(
    normalc = (alcohol - mean(alcohol))/sd(alcohol),
    normalized_alc_robust = (alcohol - median(alcohol))/IQR(alcohol),
         .keep = "none")
```
...
Here we have extreme right shift of distribution. Let's look at mean to see Mbappe effect.

```{r}
mean(white_wine$`residual sugar`)
median(white_wine$`residual sugar`)
```
...
white_wine |>
  reframe(quantile(`residual sugar`),
          mean(`residual sugar`))
```
Summarise has other functionalities (grouping) and the functionality that is interesting here is moved in reframe.
Here mean is copied bc dataframe so quantile has to be same size as mean.
SO REFRAME IS SOLUTION
...
sugar_stats = white_wine |>
  summarise(list(quantile(`residual sugar`)),
            mean(`residual sugar`))
```
This can be useful not here but fx if you have multiple subpopulations. You know heterogenerity is explanatory so you make linear model for men, linear model for men.

The lin model is a form of complex summary. Hence with reframe we can do that in a clean way.
